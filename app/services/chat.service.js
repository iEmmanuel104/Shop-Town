...
const { User, Brand, ChatRoom, Message } = require('../../models')
...
const initiateChat = async function (req, res) {
    const { data } = req
    const socket = this;
    const { targetuser_id, store_id } = data;
    ...
    const store = await Brand.findByPk(store_id);
    if (!store) {
        res.send("Store does not exist");
        return;
    }

    const store_owner = await store.owner

    // // Check if user is part of store
